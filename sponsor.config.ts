...
// Open {re}Source colours. The images are shown on other sites and on both GitHub themes, so they carry their own dark
// background. Web fonts can't load in an SVG shown with <img>, so the text falls back to system fonts.
const FIELD = '#0A0C11'
const LINE = '#292C33'
const TEXT = '#E6E8EC'
const MUTED = '#8F929A'
...
`,
  ...
  async customComposer(composer, sponsors, config) {
    ...
    composer.addSpan(14).addText('github.com/sponsors/Open-reSource', 'sponsorkit-url')
    composer.addSpan(config.padding?.bottom ?? 20)
